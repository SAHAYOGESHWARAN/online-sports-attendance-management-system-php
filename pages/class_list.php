<div class="page-title mb-3">List of sports</div>
<hr>
<?php 
                        ...
                        <table class="table table-bordered table-hovered table-stripped">
                            ...
                            <thead class="bg-dark-subtle">
                                <tr class="bg-transparent">
                                    ...
                                    <th class="bg-transparent text-center"> Name - Sport</th>
...
<script>
    $(document).ready(function(){
        $('#add_class').click(function(e){
            e.preventDefault()
            open_modal('class_form.php', `<?= isset($id) ? "Create New sport" : "Update sports" ?>`)
        })
        $('.edit_class').click(function(e){
            e.preventDefault()
            var id = $(this)[0].dataset?.id || ''
            open_modal('class_form.php', `<?= isset($id) ? "Create New sport" : "Update sport" ?>`, {id: id})
        })
        $('.delete_class').click(function(e){
            e.preventDefault()
            var id = $(this)[0].dataset?.id || ''
            start_loader()
            if(confirm(`Are you sure to delete the selected sports? This action cannot be undone.`) == true){
                $.ajax({
